...
- [ ] **`JR/DY` duration mapping** — current mapping only computes duration from `TotalPlannedMachineHours × 3600`, which is 0 for subcontract (`JR/DY`) tasks. The actual elapsed days live in `CycleTime` (qty-independent — vendor lead time). All 12 SUBCONTRACT_DAYS tasks in the slim dataset ship with `durationSeconds: 0`. Working theory: for `Formula === "JR/DY"`, `durationSeconds = CycleTime × 86400`, durationType stays FIXED, no qty multiplication (vendor turnaround is the same whether you send 1 part or 9). **2026-05-12: manually patched in `stafford-slim-100/data/tasks.json` for the demo** — 10 of 12 SUBCONTRACT_DAYS tasks got real durations (1–7 days); 2 had `CycleTime=0` and were left at 0. Next step: add a mapping rule (or pre-rule lookup) so any re-run of the transform script applies the patch automatically.
- [ ] **Subcontract resource = pooled infinite capacity** — current mapping pulls `NumOfAvgResource` into `parallelCapacity`, which is `1` for vendor resources (Genius doesn't model vendor parallelism). The calendar's `qty` field is the real concurrency knob and was authored at `99` for OUTWORK — but the engine uses `parallelCapacity` as the binding constraint. **Design decision (2026-05-12, Bob):** vendor capacity should be infinite-pooled. **Manually applied in `stafford-slim-100`:** `parallelCapacity: 9999` on resources 1/25, calendar collapsed from 365 daily 24h intervals to a single 1-year interval with `qty: 9999` (cleaner "always available" model than daily slices). Mapping rule needed: when `RessourceType === "S"`, force `parallelCapacity: 9999` and (probably) author the calendar as a single horizon-spanning interval. **Note:** even with this fix, some chains still fail to place their OUT tasks — the engine emits "Could Not Find Availability for OUTWORK" but the actual cause is chain-propagation failure earlier in the chain (separate engine bug, likely the deferred chain-FLOAT maxGap issue).

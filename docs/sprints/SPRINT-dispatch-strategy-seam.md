...
### Where this sits — A-P-T-R (admission vs. processing sequence)

> **Load-bearing note.** Admission (A) — the set of tasks/orders CTP schedules — is fixed per snapshot and upstream of the constructive loop. This sprint makes P (processing sequence) pluggable; A stays a static rule set. **Precedence is an A-readiness rule, not a P concern:** a dispatch plug reorders the admitted ready set, it never expands it. "Which sort do we use" (P) and "what's eligible to schedule" (A) are different layers and must not be conflated.

The constructive scheduler is four decisions: **A** — admission (which tasks/orders are in the problem), **P** — processing sequence (what to schedule next), **T** — time (when), **R** — resource (where). A sits above the loop and defines the universe it runs over; P-T-R solve within that universe. This sprint elevates **P** from a static sort to a pluggable dynamic dispatch seam, leaving A-T-R unchanged. A is deliberately **not** a plugin yet — a flexible solver over a fixed problem is useful; the reverse is not, so P is made pluggable first.

A carries two concerns at different time scales, unified for now under one static rule set:

- **A-inclusion** — which orders enter CTP at all (open-WO filtering, the `9999`-complete convention, horizon bounds), resolved **once** at sync/hydrate; and
- **A-readiness** — of included tasks, which are eligible this round (all-predecessors-scheduled, material-ready, pins), re-evaluated **each round** as placements change.

Every precedence/eligibility relaxation a client may later want — soft precedence, Genius one-prior-complete, material gating, subset scheduling, overlap/lag — is an **A** change, not a **P** plug. When A eventually becomes pluggable it will likely split along this inclusion/readiness fault line; today it stays hardcoded, with **all-preds-scheduled** as the correct default readiness predicate. The invariant that keeps P safe regardless of which dispatch rule runs: **a plug reorders the ready set, never expands it** — so no sort, however greedy, can violate precedence, because precedence is enforced in A before P ever sees a task.
...
| DBR | `DBRDispatchPriority` | default | **✅ Built this sprint** — down-ranks bottleneck-bound work; constraint identified once per solve. |
| Slack/CR (Stafford strategy) | `SlackDispatchPriority` | default | **✅ Built this sprint** — least-slack-first, `slack = deliveryDate − asOf − remainingWork`; customer date preferred, `dueDate` fallback. Kaleb/Logan's rule. |
...
**Phase 0.5 — Minimum customer-date model (data hygiene; build + smoke test).**
Foundational — get the customer date into the model before any consumer reads it; small, and lives in this sprint. **One nullable field:** `customerDeliveryDate?: number` (epoch, like `dueDate`) on the work order (`IWorkOrderData` + task hydration), **inherited-not-authored** — read-only, stamped from source, never user-edited per WO (say so in the doc comment; the future demand-driver becomes the *author* and this field its inherited copy — additive, no migration). **One mapping line:** `DeliveryDate → customerDeliveryDate` at the single mapping point (source = `WorkOrderWithAdvancedInformationViewEntity.DeliveryDate`, already pulled — today it lands in `lateDueDate`/`promiseDate`). **`dueDate` stays `= JobEndDate`** — the scheduler keeps targeting the internal, buffer-adjusted finish-by. **Null policies (decide now — two consumers):** (i) **KPI** — a null-date WO is excluded from **both** numerator and denominator of the signed-gap / on-time rollups (measure only the population that *has* a customer date, so stock/internal work can't dilute % on-time); (ii) **Slack/CR** — a null-date task still schedules but has no customer pressure → fall back to max-slack / lowest customer-urgency (see Phase 3.5). **Keep clean of the Work7 ASAP/SO-date issue** — the field's existence and semantics are independent of that data-quality question; Work7 resolves *what value feeds it*, not whether the slot is right. **Regenerate** the full dataset through the updated mapping and derive slims from it (slim-500/1000/2000 disposable; keep full + slim-100). **Smoke test:** `customerDeliveryDate` hydrates on customer WOs, is `null` on internal/stock WOs, and the signed-gap rollup excludes nulls. **Future-state, additive (not here):** group-level field, demand-driver (SalesOrder) entity, priority-on-driver.

**Phase 1 — The seam + default plug (behavior-preserving). ✅ DONE.**
*Shipped:* `IDispatchPriority` (comparator) + `DispatchState` lens + `DynamicNeighborhood` + `StaticRankPriority` (chain-head sort `(rank, window.startW)`; presents as strategy name `"Chain"` for backward-compat). `resolveStrategy` default + compatibility guard now resolve to `DynamicNeighborhood(StaticRankPriority)`. **Parity gate green byte-for-byte** across all 3 tenants; `ChainNeighborhood` retired (deleted, index + 2 comparison tests migrated). Full suite 1252 pass, strict tsc clean. The standalone `greedySortFn` backfill is preserved in `DynamicNeighborhood` (dead path — no tenant has standalone tasks).
...
**Parity gate — ✅ BUILT** (`schedule-parity.spec.ts` + committed `__parity__/*.golden.json`): solves each tenant twice (determinism guard) → serializes `{key,start,end,resources}` per task → deep-equals a committed golden. Findings: (i) **no eval-clock dependency** — every tenant has a fixed-date horizon, so the constructive solve is already deterministic (no `asOf` needed); (ii) **no standalone tenant exists** (Phase 0e), so the chain-head sort is the whole parity surface; (iii) **3 tenants excluded** — `demo-manufacturing` / `summit-pharma` / `stafford-engineering` run the engine's optimization layer during solve, which is **non-deterministic by design** (`perturbation.ts` uses `Math.random()`; `tabusearch.ts` terminates on a wall-clock time budget), so they cannot back a byte-for-byte gate. The gate covers the 3 deterministic constructive tenants — slim-100 (cross-WO), acme (chains), hrmd (scale) = 266 tasks. Once `DynamicNeighborhood(StaticRankPriority)` clears this gate, **retire `ChainNeighborhood` in this same deliverable**. No new behavior ships in this phase.
...
**Phase 3 — DBR plug. ✅ DONE.**
`DispatchState` gained memoized `resourceLoad()` — total required processing time per primary resource key over the landscape's tasks — plus an exported `primaryResourceKey(task)` helper (reads `capacityResources`, primary-else-first `resource` key; available pre-schedule, unlike `scheduledResource`). `DBRDispatchPriority.prepare()` argmaxes `resourceLoad()` **once per solve** (cached on the plug; `undefined`→uncomputed, `null`→no constraint), lowest key breaks a load tie for determinism. v1 semantics: **down-ranking, not gating** — `compare()` sinks bottleneck-bound tasks (primary resource == drum) behind non-bottleneck tasks; within a class it falls back to the byte-for-byte static order (rank, then window), so DBR is a pure reordering of the default and never stalls (all-bottleneck-bound → one is released by construction). Wired `resolveStrategy` `case 'DBR'`. Tests (`dbr.test.ts`): (i) bottleneck-bound work deprioritized behind non-bottleneck even when due-urgent; (ii) all-bottleneck-bound still releases (no stall); (iii) constraint identified deterministically (max load, lowest-key tie-break); (iv) empty load → pure static order. **Parity gate still byte-for-byte** (the lens extension is memoized/additive; ATC + default unaffected). Full suite 1259 pass.

**Phase 3.5 — Slack/CR plug (Stafford's rule — the first real consumer). ✅ DONE.**
`SlackDispatchPriority`: priority = **least slack first**, `slack = deliveryDate − asOf − remainingWork`, with `asOf = state.now()` (shared with ATC) and `remainingWork` = task duration (v1 coarsening). **No new `DispatchState` accessor** — one class + one registry line, reusing `now()` and the Phase 0.5 `customerDeliveryDate` task field. Since `asOf` is constant across a round, this ranks by `deliveryDate − remainingWork`, so at equal delivery dates the heavier job (less slack) leads. **Date source:** `customerDeliveryDate` (customer promise) preferred; falls back to internal `dueDate`; a task with neither date sorts last (max slack) but **still schedules** (never dropped). Within equal slack the tie-break is the static order (rank, then window). *(Note: the `dueDate` fallback and the "dateless sorts last via max-slack" sentinel described here were **removed in Phase 3.9** — null customer date is now a backfill class, not a late date.)* Full suite 1265 pass.

**Phase 3.9 — Two-class dated/backfill partition + order-sourced dates. ✅ DONE.**
Reworked the plugs onto a shared **two-class partition** so "no demand date" is modeled as a *class*, not a sentinel (different physics: stock/internal work fills white space, it is not "very late"). Added `governingDate(state, task): number | null` to `IDispatchPriority` (per-plug datedness) and a `BaseDispatchPriority` base whose `compare` partitions once for all plugs: *dated ranks before backfill; both-backfill → `legacyCompare` (the `(rank, window.startW)` parity anchor); both-dated → the plug's `compareDated`*. Slack/index arithmetic is unreachable for null-governed work by construction — **no `MAX_SAFE_INTEGER`/end-of-horizon anywhere.** Per-plug axis: Slack→`customerDeliveryDate` (**no `dueDate` fallback**), ATC→`dueDate`, DBR→non-null always (never backfill), Static→null always (all-backfill → legacy, so parity holds).
- **Dates read off the ORDER** (`task.linkId.name` → `landscape.orders`), not the task — a chain head's own `dueDate`/`customerDeliveryDate` are 0 (`hydrateDueDates` stamps sinks only). Lens accessors `dueDateOf`/`deliveryDateOf`/`penaltyOf`; the earlier `terminalOf`/`succs`-walk removed.
- **`asOf()` = `horizon.startW`** (the fixed snapshot clock, the ClockService `asOf` materialized) replaces `now()` in ATC/Slack; stable per solve, sits inside ATC's `exp()` so it governs ordering. No engine↔ClockService coupling.
- **Registered** `ATC`/`DBR`/`Slack` in `DISPATCHING_STRATEGIES` (`isPublic: true`, beta — available to all tenants; selectable via `solve({strategy})`).
- **Backfill data:** `customerDeliveryDate` nulled on non-`C` (internal/stock/quote) orders in slim-100 (6) and slim-2000 (103) — the correct mapping output (`DeliveryDate` where `jobType='C'`), not a contrived fixture.
- **Tests:** `dispatchpriority`/`partition.test.ts` — the **discriminator** (`dueDate` present + `customerDeliveryDate` null → backfill under Slack *and* dated under ATC; catches a shared-`isDated` regression); per-plug `governingDate`; Slack + ATC backfill; both-null→legacy tie. Parity still **byte-for-byte** (Static → all-backfill → `legacyCompare`).
...
- **`customerDeliveryDate` mapping (definition + live wiring)** — the field's true source is Genius `WorkOrderWithAdvancedInformationViewEntity.DeliveryDate`, populated **only where `jobType = 'C'`** (a customer commitment); null otherwise (internal/stock → backfill). The slim tenants' data now reflects this (non-`C` nulled). The live `stafford-engineering-test` mapping already sets `customerDeliveryDate` ← `DeliveryDate` **unconditionally**; whether that already yields null for non-`C` depends on whether Genius leaves `DeliveryDate` blank on internal jobs — **verify on the next capture**. If Genius populates it on non-`C`, add a `jobType='C'` guard (a small field-level-conditional modifier the mapping engine doesn't have yet; adjacent to the filter-support work). *Naming cleanup:* `lateDueDate` and `customerDeliveryDate` currently both map from `DeliveryDate` — retire `lateDueDate` in favor of `customerDeliveryDate` as the single home once the seam settles.
- **`GreedySortPriority` plug** — the greedy 5-key comparator (`earliestEnd, score, rank, window.startW, duration`) as a date-orthogonal plug (`governingDate` non-null always, like DBR). Would give *chain-safe* greedy (greedy ordering of chain heads) — not byte-for-byte with the chain-blind `GreedyNeighborhood`, so a deliberate migration, not a swap. Also the natural comparator to drop into the both-backfill branch if backfill should pack greedily rather than sit in chain-priority order. Not built here.
- **Signed-gap KPI null semantics (forward requirement for the KPI sprint)** — the customer-date gap rollup must treat a null `customerDeliveryDate` as an **inner-join drop** (no customer date → no penetration to measure → the row leaves the gap population), never a left-join coercing null → 0/epoch. This seam guarantees the *upstream* honesty — the lens returns real `null`, no sentinel — but enforcing inner-join at the *KPI* layer is the bake-off/KPI sprint's obligation, not this sprint's code. Recorded here so it isn't lost.

---

## Field-dependency ordering (context — read before judging a "backfill everywhere" result)

The two-class partition keys off `deliveryDateOf(task)` → the order's `customerDeliveryDate`. That field only exists in the ready set **after** it has been hydrated onto the order at sync/ingest. So on a tenant (or a harness fixture) where `customerDeliveryDate` **isn't hydrated yet**, *every* task resolves to `null` → the whole ready set is the **backfill class** → Slack reduces to the both-null legacy comparator, i.e. it matches the default. **This is expected, not a failure** — it's the honest consequence of "no demand date present," and it's exactly why the lens must return `null` and never a sentinel. A "Slack == default on tenant X" result therefore means "tenant X has no customer dates in scope," not "the plug is inert." Verify Slack's *dated* behavior on a tenant/fixture that actually carries `customerDeliveryDate` (see open item on backfill coverage).

---

## Acceptance Criteria

- [x] `IDispatchPriority` + `DispatchState` exist; `DispatchState` is **read-only** over the landscape (enforced by type and test) with **memoized-per-round** shared accessors and a raw-landscape escape hatch.
- [x] `DynamicNeighborhood(StaticRankPriority)` produces **byte-for-byte identical** schedules to the current engine across the standard tenant set (the parity gate); the extracted `legacyCompare` is the both-backfill branch.
- [x] The **default sort is a plug** — no separate static code path; static vs. dynamic differ only by which accessors a plug reads. **`ChainNeighborhood` retired.**
- [x] **Dynamic plugs run at N=1**; the two-class partition + per-plug `governingDate` replaces any shared datedness flag. *(FORWARD/BACKWARD scoping unchanged from the default path.)*
- [x] `ATCDispatchPriority` is selectable per solve, reorders by live slack (unit-tested), and does **not** alter the default. v1 remaining work = task duration (documented coarsening).
- [x] `DBRDispatchPriority` **down-ranks** bottleneck-bound work against a once-per-solve constraint index built in `prepare()` (unit-tested), **never stalls** on an all-bottleneck-bound ready set (unit-tested), and leaves ATC + default unaffected.
- [x] `SlackDispatchPriority` ranks by least customer-delivery slack; null customer date = **backfill** (no `dueDate` fallback). The **discriminator** test (`dueDate` present / `customerDeliveryDate` null → backfill under Slack, dated under ATC) passes.
- [x] **No sentinel coercion anywhere in this sprint** — the lens returns honest `null`; null is the backfill *class*, never a value. *(KPI-layer inner-join is a recorded forward requirement — see Out of Scope.)*

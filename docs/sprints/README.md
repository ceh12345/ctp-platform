...
| Item | Summary | Date |
|------|---------|------|
| [Staging Architecture](sprint-staging-architecture.md) | ETL staging layer between adapter and engine, shipped as 4 milestones. **M1** — StagingService + pointer abstraction (SymlinkPointer Linux/Docker, JunctionPointer Windows dev) + ValidationRunner with 4 rules (record-count plausibility, required-fields, date-parseability, cross-entity-refs). **M2** — SyncOrchestrator + StagingReadAdapter + `ConfigService.getStagingConfig()` + flag-gated `SyncService.sync()` delegation + `AdapterFactory` `staging-read` branch. **M3** — `StagingLifecycleService` (orphan cleanup on init + 24h retention prune via `setInterval`) + CLI `list/inspect/promote/rollback` + Docker volume doc. **M4** — `staging seed` CLI subcommand for one-shot fixture-to-snapshot conversion (camel + kebab filename variants; defaultRules validation gate); `stafford-slim-100-staging-test` tenant config (cloned from `stafford-slim-100` with `integration/adapter.json` = staging-read); kill-9 mid-sync recovery test; end-to-end seed→sync test. Sprint draft: [`sprint-staging-architecture.md`](sprint-staging-architecture.md). Design rationale: [`staging-architecture-design.md`](staging-architecture-design.md). 1131 tests pass (was 982 pre-sprint; +149 new tests across staging + validation + CLI + lifecycle + activation). | 2026-05-23 |
...
*Last updated: May 12, 2026 (Up Next reordered — Staging Architecture → Cycle Time → Code Optimization Engine Runtime at the top of the queue)*

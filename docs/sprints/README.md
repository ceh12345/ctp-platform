...
| Item | Summary | Date |
|------|---------|------|
| [Staging Architecture](sprint-staging-architecture.md) | **Beta: engine reads validated CTP-shape data via a per-tenant `data/current` symlink; all transformation is upstream, engine never maps.** Each tenant has committed CTP-shape data under `data/initial-fixture/`; `StagingLifecycleService` creates the `data/current` junction/symlink at boot (not committed — Windows+git); `FileConfigStore` reads through `data/current/<entity>.json` with fallback to `initial-fixture/`. `GET /v1/health/tenant` reports config/data-layout/entity/engine health for deploy verification. Initial impl (git history M1–M4) built mapping-at-read (Option A: `StagingReadAdapter`, `SyncOrchestrator`, `staging.enabled`); **pivoted** to mapping-upstream — all of that retired (deleted, grep-verified): the engine must never transform data, all ETL is upstream even in beta. Substrate kept for future cleanse tool: pointer abstraction, `StagingService` lifecycle, `ValidationRunner`+4 rules, `_history.log`, CLI (`list/inspect/promote/rollback/seed/history`). Deferred (not blocking beta): `npm run cleanse` offline tool, decoupled promote-as-job, live Genius sync. As-built doc: [`sprint-staging-architecture.md`](sprint-staging-architecture.md). 1130 tests pass. | 2026-05-27 |

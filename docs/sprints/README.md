...
| Sprint | Name | Notes |
|--------|------|-------|
| [Engine — Pred/Succ Edge-List Refactor](sprint-engine-edge-list-refactor.md) | Replace implicit `i±1`/single-`prevLink` precedence with explicit `preds[]`/`succs[]` edge lists | **Spec ready.** Behavior-preserving structural refactor — every precedence decision becomes `max(pred ends)` / `min(succ starts)`; length-1 lists == current linear behavior, so the regression suite is the safety net (output must stay byte-identical on all tenants). Foundation that turns parallel WO Groups into a *producer + semantics* change later, not an engine restructure. ~14 core sites across `chaincontextengine`/`basescheduler`/`landscape` + ~5 selection sites; **only `assignStartTimes` (single-spine anchor → topological fill) is non-mechanical.** Reuses disjunctive-graph adjacency + Kahn topo sort. No tenant data changes. ~3.5-4 days. **Sequenced after engine-optimization** — overlap is confined to `chaincontextengine.ts` (opt branch's forward-pass + `assignStartTimes` hunks collide with this refactor's heaviest rewrites); land optimization first, then refactor the optimized code (no merge). Independent of Stafford answer (parallelism *enablement* is the follow-on). |
...
| [Cycle Time on Tasks](sprint-cycle-time.md) | Per-unit `cycleTime` field on CTPTask | Preserves original standard duration (cycle time) separately from active scheduling `duration`. Stored as **seconds per unit** matching Stafford's `Formula: "HR/UN"`; `theoreticalDurationSeconds() = cycleTime × qty` derived helper. Null falls back to `duration.duration()`. Mapping rule from Genius `CycleTime` field with 0/null passthrough for design/QC tasks. API ships `cycleTimeSecondsPerUnit` (raw) + `theoreticalDurationSeconds` (derived); UI task detail shows both when distinct from scheduling duration. Unblocks CTP queries against in-progress order chains. Pairs with Stafford v3.2 FLOAT mapping flip. Spec: `sprint-cycle-time.md`. |
| [Code Optimization — Engine Runtime](CODE-OPTIMIZATION-SPRINT.md) | Remove quadratic + redundant work from scheduling hot paths | 16 ranked tickets across `chaincontextengine`, `availableengine`, `starttimeengine`, `scoringengine`, `intervals`, `setengine`, `disjunctivegraph`, `combinationengine`. P0 inner-loop fixes (5): tail-merge in `feasibleStartTimes`, qty-indexed `addToFloat`, typed-array+binary-search StartTimes cache, score-unique-contexts-once, resource-indexed `detectLanes`. P1 warm path (7): BaseX cross-product, lazy `resourceHash`, hoisted cumulative durations, tail-first list `add`, scoring-rule map, `addRunRates` head-only bug, `mergeAvailables` linkedlist scan. P2 cleanup (4) including a `combinationengine` `str.includes` false-positive bug fix. Unified version-based cache invalidation; benchmark gate (no >5% regression on any fixture). ~5.5 days across 3 weeks. |
...
/docs/sprints/
  ...
  sprint-engine-edge-list-refactor.md ← Spec: replace implicit i±1/prevLink precedence with explicit preds[]/succs[] edge lists (behavior-preserving); call-site inventory, phases, assignStartTimes redesign as the only hard area
...
*Last updated: June 17, 2026 (added Pred/Succ Edge-List Refactor spec + Parallel Processes design note to Up Next + File Index)*

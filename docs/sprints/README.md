...
| Sprint | Name | Notes |
|--------|------|-------|
| [Staging Architecture](sprint-staging-architecture.md) | ETL staging folder + atomic-promotion symlink | Layer between "what Genius API returned" and "what the engine consumes." Timestamped snapshot folders under `staging/<tenant>/`, `current` pointer (symlink on Linux/Docker, junction on Windows dev) for atomic promotion, `_metadata.json` + `_validation-report.json` per snapshot. Enables pre-live cleansing, instant rollback, audit trail, sync/process decoupling. New `StagingService` + `SyncOrchestrator` + `StagingReadAdapter`; `SyncService.sync()` delegates when `staging.enabled`. v1 cleansing: Phase 1 validation only (record-count plausibility, required fields, date parseability, cross-entity refs); promotion gate fails on required-fields or cross-entity-refs. Retention 30 days, daily prune (never deletes `current` target). CLI: `list` / `promote` / `inspect` / `rollback`. Sprint draft: `sprint-staging-architecture.md`; design rationale: `staging-architecture-design.md`. ~1.5 days, 6 phases. |

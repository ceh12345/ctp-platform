...
| Sprint | Name | Notes |
|--------|------|-------|
| [Move the Solve Off the Main Thread](SPRINT-solve-off-main-thread.md) | Solve blocks the event loop — the whole site is deaf while it runs | **Measured 2026-08-30.** The engine solve is synchronous CPU work on Node's single thread, so during a solve the API answers nothing: a `/v1/health` poll waited **51.6s** mid-solve, and on Azure even `index.html` failed to load. Blackout ≈ solve time: **~400s** on the small tier, **73s** scaled up, ~50s locally. Scaling shortens it but never removes it. Note a `202 + jobId + poll` design fails identically — the status endpoint is just as blocked. Interim shipped (`98f3e1f`): `solveWithRecovery()` recovers a result when the request dies, but carries no progress heartbeat because nothing can observe progress. Recommended fix: **a second process of the same image** as a solver (~1 day — the solver is the same image on another port, and the snapshot reload path already exists and is tested), rather than a worker thread (2–4 days: `solve()` mutates a live object graph bound to Nest DI, which is not structured-cloneable). Matters most for the **client Docker deployment** — Stafford's box can't be scaled on demand. Also corrects an assumption made during diagnosis: **no 230s Azure cut was ever observed** — a 400s request completed intact. |
...
| [Orders Page — Delivery Projection from CTP's Own Schedule](SPRINT-orders-delivery-projection.md) | Orders grid delivery risk now derives from **CTP's projection**, not Genius dates. Old rule compared the parent Job's `sourceEnd` to wall-clock and never consulted the schedule CTP had just produced — on the 16 July book that was **101 false LATE + 31 false AT_RISK**. New: order **projected span** (earliest scheduled start / latest scheduled end across its tasks) emitted on `OrderResultDto` from `ctp.service`, and joined at read time in `orders.service` via `projectedSpans()` off the solved landscape (never stored — solve output, consistent with the snapshot sprint). Status = LATE (past the END of the promised day) / AT_RISK (≤5 days slack) / ON_TRACK / **null** where unassessable (no customer promise, or nothing scheduled — 138 of 472 rows); COMPLETED/CANCELLED still win from `wostatus`. Promise is **midnight at the START** of the promised day (518/519 values are 00:00 NZ) so the deadline is **+24h** — fixed 8 same-day completions previously marked "+1". Four new columns (Customer Promise, Projected Start, Projected End, Days Late); `SolveResultsDialog` late-orders re-based off the promise instead of `dueDate` (Genius `JobEndDate`, differs on ~87% of orders, set even on internal/stock/rework). **Three bugs fixed en route:** blanks floated to the TOP when sorting descending (sign applied to compareValues' empty-last, burying all 334 populated rows under 138 empties — no prior column had blanks); `resolveValue` silently no-op'd on the new columns while the header still drew a sort arrow; and **pagination was pinned to page 1** (`clampPositiveInt(page, 1, 1)` — the third arg is a MAX — so only the first 100 of 472 orders were ever reachable). New `orders.service.spec.ts`, **45 tests**, mutation-verified (reverting the fixes fails 8 and 4 respectively). Suite 1393/10, strict tsc clean, all three packages build. Verified end-to-end on `stafford-all`: 174 ON_TRACK / 107 LATE / 53 AT_RISK / 138 —, worst `28482` +203d. **Open:** `AT_RISK_SLACK_DAYS=5` hardcoded pending Stafford input; columns blank until a solve runs in-process; not yet demoed. | 2026-08-12 |

...
## Part 4: Chat Auto-Collapse on Action Click

### The problem
The chat panel competes for screen real estate with the Gantt and task/resource detail
panels. When a planner clicks an action button — "Show options on Gantt" — the WhereTo
ghost bars appear on the Gantt, but the chat panel is still covering part of it.
Closing the chat loses the conversation. Leaving it open obscures what the action just revealed.

### The solution — auto-collapse to a strip
When an action button is clicked, the chat panel collapses to a thin strip on the right
edge of the screen. The strip shows just the last AI message as a preview. The Gantt
and any open detail panels expand to fill the space. The planner clicks the strip (or
a re-open button) to restore the full chat panel — conversation history intact.

```
Full chat open:                    After action click:
┌──────────────┬───────────┐      ┌─────────────────────┬──┐
│              │  AI Chat  │      │                     │≡│
│    Gantt     │           │  →   │       Gantt         │ │
│              │ [📍 Show] │      │    (full width)     │A│
...
Add `chatCollapsed` state to the chat panel (or lift to App if the panel width
is already controlled from App):

```typescript
const [chatCollapsed, setChatCollapsed] = useState(false);
```

### 4b. Collapse on action

In `handleChatAction` in App.tsx, collapse the chat immediately after dispatching
the action:

```typescript
const handleChatAction = useCallback((action: ChatAction) => {
  // Dispatch the action (existing switch statement)
  switch (action.type) {
    case 'whereTo': ...
    case 'openTask': ...
    // ... etc
  ...
  // Collapse chat after any action — give the UI room to show the result
  setChatCollapsed(true);

}, [...deps, setChatCollapsed]);
```

### 4c. Collapsed strip component

The collapsed strip renders as a narrow vertical bar on the right edge:

```typescript
function ChatCollapsedStrip({
  lastMessage,
  onExpand,
}: {
  ...
  return (
    ...
    >
      ...
      {/* Last message preview — rotated vertically */}
      {lastMessage && (
        <div style={{
...
### 4d. Toggle in chat panel render

The chat panel component switches between full and collapsed based on `chatCollapsed`:

```typescript
// In AIChatPanel render:
if (chatCollapsed) {
  return (
    <ChatCollapsedStrip
      lastMessage={messages.filter(m => m.role === 'assistant').pop()?.content ?? null}
      onExpand={() => setChatCollapsed(false)}
    />
  );
}

// Otherwise render full chat panel as before
return (
  <div style={{ width: 340, ... }}>
...
### 4e. Re-expand behavior

Clicking the strip re-expands the full chat panel. No state is lost — conversation
history, pending input, everything is preserved. The chat resumes exactly where it
left off.

The planner's natural flow becomes:
```
1. Ask question in chat
2. AI responds with action buttons
3. Click action → chat collapses → Gantt shows result
4. Examine the result
5. Click the strip → chat re-expands → continue conversation
```

### 4f. Don't collapse for openTask / openResource

`openTask` and `openResource` open slide-over panels that don't need the full Gantt width.
For those action types, skip the collapse — the chat and task panel can coexist:

```typescript
// In handleChatAction, only collapse for space-hungry actions:
const shouldCollapse = ['whereTo', 'filterChain', 'openTab', 'navigateOrder']
...
**Chat collapse behavior:**
- [ ] Action click collapses chat to 32px strip on right edge
- [ ] Strip shows ✦ icon + last AI message preview (rotated vertical text)
- [ ] Strip has hover state
- [ ] Clicking strip re-expands full chat — conversation history intact
- [ ] `openTask` and `openResource` do NOT collapse chat
- [ ] `whereTo`, `filterChain`, `openTab`, `navigateOrder` DO collapse chat
- [ ] Gantt expands to fill freed space when chat collapses
- [ ] Transition is smooth (no jarring layout jump)
...
- Collapsed strip component + toggle logic: ~15 min
- Selective collapse by action type: ~5 min
- Testing: ~5 min
- **Total: ~95 min**

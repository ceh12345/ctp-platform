...
export class TabuList {
  ...
  isTabu(move: NeighborhoodMove, currentIter: number): boolean {
    for (let i = this.entries.length - 1; i >= 0; i--) {
      ...
      // Early exit: entries older than tenure can't match
      if (currentIter - e.iteration >= this.tenure) continue;
...
export function evaluateMove(
  ...
  if (graph.hasCycle()) {
    // Reverse swap (after swap, B is before A, so reverse is swapping B,A)
